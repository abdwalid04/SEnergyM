...
class UKDALE(Dataset):
    def __init__(self, ROOT_DIR, UKDALE_DIR, house_id, fold, appliances, window_size=100, slide_size=50):
        self.fold = fold
        ...
        labels_path = os.path.join(UKDALE_DIR, f"house_{house_id}", 'labels.dat')
        meta_path = os.path.join(UKDALE_DIR, f"metadata/building{house_id}.yaml")
        ...
        json_data = requests.get(meta_path)
        house_meta = yaml.full_load(json_data.text)
